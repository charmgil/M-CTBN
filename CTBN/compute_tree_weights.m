...
for i=1:length(T)
    ...
    if(isempty(T{i}.parent))
        ...
        if ~is_weighted
            ...
            elseif isfield(T{i}, 'best_cost')
                ...
                [T{i}.weights, learned_best_cost] = LR_train(X, Y(:,node), is_learn_cost);
            end
        else
            % weighted LR
            if ~isempty(user_cost)   % cost in user param has higher priority
            ...
            elseif isfield(T{i}, 'best_cost')
                ...
                [T{i}.weights, learned_best_cost] = LR_train(X, Y(:,node), w*N, is_learn_cost);
            end
        end
        ...
        if is_switching
                ...
                if sum(roi) > 3
                    if ~is_weighted
                        ...
                        elseif isfield(T{i}, 'best_cost') && ~isnan(T{i}.best_cost(j))
                            ...
                            [T{i}.weights{j}, learned_best_cost(j)] = LR_train(X(roi,:), Y(roi,node), is_learn_cost);
                        ...
                        elseif isfield(T{i}, 'best_cost') && ~isnan(T{i}.best_cost(j))
                            ...
                            [T{i}.weights{j}, learned_best_cost(j)] = LR_train(X(roi,:), Y(roi,node), w(roi)*N, is_learn_cost);
                            is_learned_best_cost(j) = true;
                        end
                    end
                else
            ...
            if ~is_weighted
                ...
                elseif isfield(T{i}, 'best_cost')
                    ...
                    [T{i}.weights, learned_best_cost] = LR_train([X Y(:,parent)], Y(:,node), is_learn_cost);
                end
            else
                % weighted LR
                if ~isempty(user_cost)   % cost in user param has higher priority
                ...
                elseif isfield(T{i}, 'best_cost')
                    ...
                    [T{i}.weights, learned_best_cost] = LR_train([X Y(:,parent)], Y(:,node), w*N, is_learn_cost);
                end
            end
    ...
    if is_learn_cost && ~isempty(learned_best_cost)
        if ~is_switching || isempty(T{i}.parent)
            T{i}.best_cost = learned_best_cost;
        else
            for j = 1:T{i}.card
                if is_learned_best_cost(j)
                    T{i}.best_cost(j) = learned_best_cost(j);
                else
                    T{i}.best_cost(j) = NaN;
                end
            end
