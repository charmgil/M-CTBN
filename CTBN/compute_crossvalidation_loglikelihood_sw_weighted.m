...
for i=1:k
    ...
    if sum(roi) > 4
        [ model0 ] = LR_train( X_train(roi,:), Y_train(roi,:), W_train(roi,:), learn_cost );
        roi0 = (Y_parent_validation==0);
        if sum(roi0) ~= 0
            P0 = LR_predict(model0,X_validation(roi0,:));
        end
    else
        no_instance0 = true;
    end
    ...
    if sum(roi) > 4
        [ model1 ] = LR_train( X_train(roi,:), Y_train(roi,:), W_train(roi,:), learn_cost );
        roi1 = (Y_parent_validation==1);
        if sum(roi1) ~= 0
            P1 = LR_predict(model1,X_validation(roi1,:));
        end
    else
        no_instance1 = true;
    end
    ...
    if no_instance0    % W0 hasn't been trained -> use W1
        roi0 = (Y_parent_validation==0);
        if sum(roi0) ~= 0
            P0 = LR_predict(model1,X_validation(roi0,:));
        end
    elseif no_instance1    % W1 hasn't been trained -> use W0
        roi1 = (Y_parent_validation==1);
        if sum(roi1) ~= 0
            P1 = LR_predict(model0,X_validation(roi1,:));
        end
    end
    
    if sum(roi0) == 0
        LL_temp(i) = LR_likelihood_weighted( P1, Y_validation(roi1,:), W_validation(roi1,:) );
    elseif sum(roi1) == 0
        LL_temp(i) = LR_likelihood_weighted( P0, Y_validation(roi0,:), W_validation(roi0,:) );
    else
        LL_temp(i) = LR_likelihood_weighted( P0, Y_validation(roi0,:), W_validation(roi0,:) ) + LR_likelihood_weighted( P1, Y_validation(roi1,:), W_validation(roi1,:) );
    end
    ...
    if length(unique(Y_parent)) == 3
        ...
        if sum(roi) ~= 0
            [ model2 ] = LR_train( X_train(roi,:), Y_train(roi,:), W_train(roi,:), learn_cost );
            roi2 = (Y_parent_validation==2);
            if sum(roi2) ~= 0
                P2 = LR_predict(model2,X_validation(roi2,:));
            end
        else
            no_instance2 = true;
        end
        ...
        if sum(roi2) ~= 0
            LL_temp(i) = LL_temp(i) + LR_likelihood_weighted( P2, Y_validation(roi2,:), W_validation(roi2,:) );
        end
